# 在 ChatGPT 中使用 Siuyu Lightbox Still Life
...
请使用当前对话可用的图像功能完成成图；若只能处理文字，请给出完整提示词。
```

也可以先发送仓库主页，请模型打开本文件。读取失败时，把本文件全文复制进对话，再附上任务。链接的作用是提供当前对话的创作上下文；重复使用时，可以重新附上文档。

## 对话中的工作方式

先读取用户要求和当前能访问的材料。能查看的附件逐张观察；需要但无法查看的图片或文档应明确说明。普通 Chat 可以直接整理自然语言提示词；有文件和代码执行能力时，可以进一步保留方向、任务及看图记录，实际执行过的步骤据实说明。
...
默认制作垂直俯拍的光台研究静物。相机光轴垂直台面，画面四周为连续的承载面。主体先被看见，设计稿、材料样片和有用途的工具围绕主体形成清楚的观看顺序。
...
底光从乳白扩散面向上穿过薄纸或半透明材料。单层纸呈现自身灰度，交叠区域的透射密度更高，覆盖区域内的下层铅笔线条更淡。纸边、单层、叠层和下层线稿共同表达透光。
